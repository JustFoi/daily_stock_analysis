...
- [修复] Pushover 推送分段发送时，单个段落超过 1024 字符会整段超限导致消息被截断；现在超长段落会优先按换行、必要时按字符强制切片，保证每条消息不超限且内容完整送达。
...
- [改进] GitHub Actions 每日股票分析定时触发由每工作日北京时间 18:00 一次，调整为 14:30、15:30 各一次（UTC cron `30 6` / `30 7`）；中英部署文档与完整指南同步更新，本地 `SCHEDULE_TIME` / `SCHEDULE_TIMES` 默认值不变。
