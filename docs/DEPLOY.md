...
默认配置：**周一到周五，北京时间 14:30 与 15:30** 各自动执行一次

修改时间：编辑 `.github/workflows/00-daily-analysis.yml` 中的 cron 表达式（可配置多行，每行一个时间点）：

```yaml
schedule:
  - cron: '30 6 * * 1-5'  # UTC 时间，+8 = 北京时间 14:30
  - cron: '30 7 * * 1-5'  # UTC 时间，+8 = 北京时间 15:30
```

常用 cron 示例：
| 表达式 | 说明 |
|--------|------|
| `'30 6 * * 1-5'` | 周一到周五 14:30（北京时间） |
