...
默认每个工作日 **14:30、15:30（北京时间）** 各自动执行一次。
...
```yaml
schedule:
  # UTC 时间，北京时间 = UTC + 8；多行 cron 表示每天触发多次
  - cron: '30 6 * * 1-5'   # 周一到周五 14:30（北京时间）
  - cron: '30 7 * * 1-5'   # 周一到周五 15:30（北京时间）
```
...
> 仓库默认已配置 14:30、15:30 两个触发点。`concurrency.group: stock-analysis` 且 `cancel-in-progress: false`，若 14:30 的运行尚未结束，15:30 的运行会排队等待而不会取消前一次运行。14:30 处于 A 股盘中，报告基于当时的实时行情；15:30 为 A 股收盘后。
